	...
	for k, v in pairs(符石组合) do
		...
		if aa[k] <= 2 then
			...
			elseif k == "点石成金" then
				...
				-- 常规提示(角色数据.数字id, "点石成金符石效果等级:" .. 属性值 .. " 词条个数:" .. 词条个数)
				角色数据.符石技能效果.点石成金 = 属性值
				角色数据.符石技能效果.点石成金触发概率 = 词条个数*20
			elseif k == "百步穿杨" then
				local 属性值 = 0
				local 参数表 = {
					25,
			...
			elseif k == "百无禁忌" then
				...
				for i = 1, #v do
					属性值 = 属性值 + 参数表[v[i]]
					-- 常规提示(角色数据.数字id, "百无禁忌符石命中等级:" .. v[i])
				end
				角色数据.符石技能效果.百无禁忌 = 属性值
				-- 常规提示(角色数据.数字id, "百无禁忌符石效果:" .. 属性值)
			elseif k == "暗渡陈仓" then
				local 属性值 = 0
