...
-- @最后修改来自: baidwwy
-- @Last Modified time: 2022-10-22 19:27:11
-- @作者: baidwwy
-- @邮箱:  小巷子资源网:vvxxz.com
...
function battle_100302:初始化(战斗id)
	self.战斗id=战斗id
end

function battle_100302:战斗准备后()

end

function battle_100302:单位死亡(编号)
	if self.参战单位[编号].名称=="疯狂意识" and self.参战单位[编号].队伍==0 then
...
function battle_100302:命令回合前(回合数)
    for n=1,#self.参战单位 do
		...
		if self.参战单位[n].名称=="叶夫人" then
			if self.疯狂死亡数量 and self.疯狂死亡数量>0 then
				self.参战单位[n].灵力=self.参战单位[n].灵力-self.疯狂死亡数量*30
				self.参战单位[n].防御=self.参战单位[n].防御-self.疯狂死亡数量*60
				self.参战单位[n].法防=self.参战单位[n].法防-self.疯狂死亡数量*30
				self.疯狂死亡数量=0
				self:添加即时发言(n,"感觉清醒一点了……")
			end
		elseif self.参战单位[n].名称=="疯狂意识" and 回合数==1 then
...
function battle_100302:战斗胜利(胜利id,失败id)
	...
	for i=1,#队伍数据[队伍id].成员数据 do
		if 队伍处理类:取是否助战(玩家数据[self.进入战斗玩家id].队伍,i) == 0 then
			...
			常规提示(临时id,"#Y/你获得了"..名称)
			广播消息({内容=format("#S(看戏-一斛珠)#R/%s#Y在#R一斛珠#Y副本中成功阻止了叶夫人，因此获得了其奖励的#G/%s#Y".."#"..取随机数(1,110),玩家数据[临时id].角色.数据.名称,名称),频道="hd"})
			elseif 奖励参数<=50 then
			local 名称="召唤兽内丹"
			玩家数据[临时id].道具:给予道具(临时id,名称)
			常规提示(临时id,"#Y/你获得了"..名称)
			广播消息({内容=format("#S(看戏-一斛珠)#R/%s#Y在#R一斛珠#Y副本中成功阻止了叶夫人，因此获得了其奖励的#G/%s*2#Y".."#"..取随机数(1,110),玩家数据[临时id].角色.数据.名称,名称),频道="hd"})
			elseif 奖励参数<=80 then
			local 名称="元宵"
